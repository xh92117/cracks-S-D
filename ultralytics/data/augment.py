...
from ultralytics.data.custom_augment import CustomAugment  # 导入自定义增强类
...
class Compose:
    ...
    def __call__(self, data):
        ...
        # 记录初始键，用于调试
        initial_keys = set(data.keys()) if isinstance(data, dict) else set()
        ...
        try:
            for i, t in enumerate(self.transforms):
                ...
                # 检查转换结果
                if result is None:
                    LOGGER.warning(f"Transform {i} ({type(t).__name__}) returned None, skipping remaining transforms")
                    break
                ...
                # 键丢失检查和恢复（如果可能）
                if isinstance(data, dict) and initial_keys:
                    missing_keys = initial_keys - set(data.keys())
                    if missing_keys and hasattr(t, '_preserve_keys'):
                        # 尝试保留原始键值
                        for k in missing_keys:
                            if k in t._preserve_keys and k in initial_keys:
                                data[k] = t._preserve_keys[k]
                    
                    # 记录丢失的键
                    if missing_keys:
                        LOGGER.debug(f"Transform {i} ({type(t).__name__}) lost keys: {missing_keys}")
            
            return data
            
        except Exception as e:
            LOGGER.error(f"Error in Compose.__call__: {e}")
            # 优雅地处理错误，返回有效数据
            return data
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        for k, v in labels.items():
            if k not in ['img', 'cls', 'instances', 'ratio_pad', 'mosaic_border']:
                result[k] = v
                self._preserve_keys[k] = v
                
        # 保存ratio_pad如果存在
        if "ratio_pad" in labels:
            self._preserve_keys["ratio_pad"] = labels.pop("ratio_pad")
            
        # 提取主要数据
        img = labels["img"].copy()
        cls = labels["cls"].copy() if "cls" in labels else np.array([])
        instances = labels.pop("instances") if "instances" in labels else None
        ...
        # 更新结果
        result["instances"] = new_instances[i]
        result["cls"] = cls[i] if len(cls) > 0 else cls
        result["img"] = img
        result["resized_shape"] = img.shape[:2]
        
        # 保存mosaic_border如果存在
        if border != self.border:
            result["mosaic_border"] = border
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        # 提取新形状，同时保存rect_shape键
        if "rect_shape" in labels:
            self._preserve_keys["rect_shape"] = labels["rect_shape"]
            new_shape = labels.pop("rect_shape")
        else:
        ...
        dw, dh = new_shape[1] - new_unpad[0], new_shape[0] - new_unpad[1]  # 填充宽高
        
        if self.auto:  # 最小矩形
            dw, dh = np.mod(dw, self.stride), np.mod(dh, self.stride)  # 填充宽高
        ...
        # 处理ratio_pad键
        if "ratio_pad" in labels:
            self._preserve_keys["ratio_pad"] = labels["ratio_pad"]
            labels["ratio_pad"] = (labels["ratio_pad"], (left, top))
            
        # 如果是空字典或只处理图像，返回图像和填充信息
        if not labels or image is not None and len(labels) == 0:
            return img if image is not None else (img, (ratio, (left, top)))
            
        # 创建结果字典并复制所有非关键键
        result = {}
        for k, v in labels.items():
            if k not in ['img', 'cls', 'instances', 'ratio_pad']:
                result[k] = v
                
        # 更新标签
        result = self._update_labels(labels, ratio, dw, dh)
        result["img"] = img
        result["resized_shape"] = new_shape
        
        # 传递预留键
        if hasattr(self, '_preserve_keys'):
            for k, v in self._preserve_keys.items():
                if k not in result:
                    result[k] = v
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if 'img' in labels:
            img = labels['img']
            result['img'] = self._format_img(img)
        
        # 处理目标检测相关数据
        if 'instances' in labels and 'cls' in labels:
            cls = labels['cls']
            instances = labels['instances']
            h, w = labels['img'].shape[:2] if 'img' in labels else (0, 0)
            ...
            if self.return_mask:
                if nl:
                    ...
                    result['masks'] = torch.from_numpy(masks)
                else:
                    masks_shape = (1 if self.mask_overlap else nl, 
                                 img.shape[0] // self.mask_ratio, 
                                 img.shape[1] // self.mask_ratio)
                    result['masks'] = torch.zeros(masks_shape)
            
            # 保存类别和边界框
            result['cls'] = torch.from_numpy(cls) if nl else torch.zeros(nl)
            result['bboxes'] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
            
            # 处理关键点
            if self.return_keypoint:
                result['keypoints'] = torch.from_numpy(instances.keypoints)
                if self.normalize:
                    result['keypoints'][..., 0] /= w
                    result['keypoints'][..., 1] /= h
            
            # 处理方向边界框
            if self.return_obb:
                if len(instances.segments):
                    result['bboxes'] = xyxyxyxy2xywhr(torch.from_numpy(instances.segments))
                else:
                    result['bboxes'] = torch.zeros((0, 5))
            
            # 归一化边界框
            if self.normalize and w > 0 and h > 0:
                result['bboxes'][:, [0, 2]] /= w
                result['bboxes'][:, [1, 3]] /= h
            
            # 添加批次索引
            if self.batch_idx:
                result['batch_idx'] = torch.zeros(nl)
        else:
            # 如果没有实例数据，创建空张量
            result['cls'] = torch.zeros(0)
            result['bboxes'] = torch.zeros((0, 4))
            if self.return_mask:
                if 'img' in labels:
                    mask_h, mask_w = labels['img'].shape[0] // self.mask_ratio, labels['img'].shape[1] // self.mask_ratio
                    result['masks'] = torch.zeros(1, mask_h, mask_w)
                else:
                    result['masks'] = torch.zeros(1, 1, 1)
            if self.batch_idx:
                result['batch_idx'] = torch.zeros(0)
        
        # 保存原始实例，便于后续恢复
        if 'instances' in labels:
            self._preserve_keys['instances'] = labels['instances']
    ...
    def _format_img(self, img):
        ...
        Args:
            img (np.ndarray | torch.Tensor): Input image as a Numpy array with shape (H, W, C) or (H, W),
                                            or a PyTorch tensor with shape (C, H, W)
...
def classify_augmentations(
    ...
    if not disable_color_jitter:
        secondary_tfl.extend(
            [
                T.ColorJitter(
                    brightness=float(hsv_v),
                    contrast=float(hsv_s),
                    saturation=float(hsv_s),
                    hue=float(hsv_h),
                )
            ]
        )
    ...
    if erasing > 0.0:
        secondary_tfl.append(T.RandomErasing(p=erasing, interpolation=interpolation, inplace=False))
...
# NOTE: keep this class for backward compatibility
class ToTensor:
    """Converts image and label data to tensors."""

    def __init__(self, half=False):
        """
        Initialize ToTensor class.

        Args:
            half (bool): Whether to convert float32 to float16. Default is False.
        """
        self.half = half
    ...
    def __call__(self, data):
        ...
        Args:
            data (dict): 包含图像和标签的字典，图像在'img'键下
        ...
        if isinstance(data, dict):
            ...
            # 处理直接图像输入（保持原有逻辑）
            im = np.ascontiguousarray(data.transpose((2, 0, 1))[::-1])  # HWC to CHW -> BGR to RGB -> contiguous
            im = torch.from_numpy(im)  # to torch
            im = im.half() if self.half else im.float()  # uint8 to fp16/32
            im /= 255.0  # 0-255 to 0.0-1.0
            return im
